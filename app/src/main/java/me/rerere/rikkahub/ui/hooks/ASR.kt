...
import me.rerere.asr.providers.TencentASRController
...
private class CustomAsrStateImpl(
    ...
    private fun createController(provider: ASRProviderSetting): ASRController? {
        return when (provider) {
            ...
            is ASRProviderSetting.Tencent -> {
                if (provider.secretId.isBlank() || provider.secretKey.isBlank() || provider.appId.isBlank()) return null
                TencentASRController(context, httpClient, provider)
            }
